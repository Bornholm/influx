angular.module('Influx')
  .factory('Signaling', ['$q', '$rootScope', function($q, $rootScope) {
    ...
    var _port;
    var _address;
    var _heartbeatRate = 5 * 1000; // Every two minutes by default
    ...
    function _messageHandler(message, rInfo) {
      if(rInfo.address !== _address && rInfo.port !== _port) {
        try {
        ...
        switch(message.type) {
          case HEARTBEAT:
            console.log('Received heartbeat', rInfo);
    ...
    function _send(messageType, data) {
      if(_server) {
        ...
        _server.send(raw, 0, raw.length, _port, _address);
        return true;
      }
      return false;
    }
    ...
    service.join = function(membership, port) {
      ...
      // Set default port
      _port = port || 25890;
      _address = membership || '224.0.1.51';
      var defered = $q.defer();
      ...
      _server.bind(_port, function() {
        ...
        try {
          ...
          _server.addMembership(_address);
