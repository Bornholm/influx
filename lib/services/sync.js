  ...
  .factory('Sync', [
    ...
    function(Store, Signal) {
      ...
      app.get('/messages', function (req, res, next) {
        Store.db.info(function (err, info) {
      ...
      app.post('/messages/_bulk_docs', function (req, res, next) {
        ...
        Store.db.bulkDocs(req.body, opts, function (err, response) {
      ...
      app.all('/messages/_all_docs', function (req, res, next) {
        ...
        Store.db.allDocs(req.query, function (err, response) {
          if (err) return res.send(400, err);
      ...
      app.post('/messages/_compact', function (req, res, next) {
        Store.db.compact(function (err, response) {
      ...
      app.post('/messages/_revs_diff', function (req, res, next) {
        Store.db.revsDiff(req.body || {}, function (err, diffs) {
          if (err) return res.send(400, err);
          res.send(200, diffs);
        });
      });
      ...
      app.post('/messages/_temp_view', function (req, res, next) {
        ...
        Store.db.query(req.body, req.query, function (err, response) {
          if (err) return res.send(400, err);
          res.send(200, response);
        });
      });
      ...
      app.get('/messages/_design/:id/_view/:view', function (req, res, next) {
        ...
        Store.db.query(query, req.query, function (err, response) {
          if (err) return res.send(404, err);
          res.send(200, response);
        });
      });
      ...
      app.put('/messages/:id/:attachment(*)', function (req, res, next) {
        ...
        Store.db.putAttachment(name, attachment, rev, body, type, function (err, response) {
          if (err) return res.send(409, err);
          res.send(200, response);
        });
      });
      ...
      app.get('/messages/:id/:attachment(*)', function (req, res, next) {
        ...
        Store.db.get(req.params.id, req.query, function (err, info) {
          if (err) return res.send(404, err);
          ...
          Store.db.getAttachment(name, attachment, function (err, response) {
            if (err) return res.send(409, err);
      ...
      app.del('/messages/:id/:attachment(*)', function (req, res, next) {
        ...
        Store.db.removeAttachment(name, attachment, rev, function (err, response) {
          if (err) return res.send(409, err);
          res.send(200, response);
        });
      });
      ...
      app.put('/messages/:id(*)', function (req, res, next) {
        ...
        Store.db.put(req.body, req.query, function (err, response) {
          if (err) return res.send(409, err);
      ...
      app.post('/messages', function (req, res, next) {
        Store.db.post(req.body, req.query, function (err, response) {
          if (err) return res.send(409, err);
          res.send(201, response);
        });
      });
      ...
      app.get('/messages/:id(*)', function (req, res, next) {
        Store.db.get(req.params.id, req.query, function (err, doc) {
          if (err) return res.send(404, err);
          res.send(200, doc);
        });
      });
      ...
      app.del('/messages/:id(*)', function (req, res, next) {
        Store.db.get(req.params.id, req.query, function (err, doc) {
          if (err) return res.send(404, err);
          Store.db.remove(doc, function (err, response) {
            if (err) return res.send(404, err);
            res.send(200, response);
          });
        });
      ...
      app.copy('/messages/:id', function (req, res, next) {
        ...
        Store.db.get(req.params.id, req.query, function (err, doc) {
          ...
          doc._id = dest;
          doc._rev = rev;
          Store.db.put(doc, function (err, response) {
            if (err) return res.send(409, err);
            res.send(200, doc);
          });
        });
      ...
      function startSync(peer) {
        ...
        if(metadata && metadata.services && metadata.services.messages) {
          var remoteId = peer.address + ':' + metadata.services.messages;
          if(!(remoteId in replications)) {
            var remoteUrl = 'http://' + remoteId + '/messages';
            console.log('Replicating', remoteId);
            replications[remoteId] = Store.db.replicate.sync(remoteUrl, {
              continuous: true,
      ...
      function stopSync(peer) {
        var metadata = peer.metadata;
        if(metadata && metadata.services && metadata.services.messages) {
          var remoteId = peer.address + ':' + metadata.services.messages;
          if(remoteId in replications) {
            replications[remoteId].cancel();
            console.log('Stopped replication', remoteId)
          }
        }
      }
