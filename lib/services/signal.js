angular.module('Influx')
  .factory('Signal', ['$q', '$timeout', '$rootScope',
      ...
      if(message.uid !== _uid) {
        switch(message.type) {
          ...
          $rootScope.$apply(function() {
            var isNew = _updatePeer(message.uid, rInfo, message.data.hbr);
            if(isNew) {
              _sendHeartbeat();
            }
          });
          break;
          case MESSAGE:
            service.emit('message', message.data, message.uid, rInfo);
          break;
          case LEAVE:
          $rootScope.$apply(function() {
    ...
    function _updatePeer(peerUid, rInfo, heartbeatRate) {
    ...
    service.join = function(host, port) {

      assert(host, 'signal.join() should received a string as first argument (host IP address) !');
      assert(+port === port, 'signal.join() should received an integer as second argument (port number) !');
      ...
      // Bind socket on port
      _server.bind(_port, host, function() {
